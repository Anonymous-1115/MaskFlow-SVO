...
OXTS_DIRNAME = "data_tracking_oxts"
DATA_DIRNAME = "data"
OXTS_LINE_LEN = 30
...
def load_oxts_data(oxts_file):
    oxts_data = np.loadtxt(oxts_file)
    return oxts_data
...
if __name__ == "__main__":
    ...
    parser.add_argument("--basedir", default="/media/xiechen/NVME/benchmark/KITTI_Tracking/data_tracking_oxts")
    parser.add_argument("--output", default="/media/xiechen/NVME/benchmark/KITTI_Tracking/data_tracking_poses")
    args = parser.parse_args()

    for split in ['training', 'testing']:
        # pose_gt file for each sequence
        oxts_files = glob.glob(os.path.join(args.basedir, split, 'oxts', '*.txt'))
        oxts_files.sort()

        # for each sequence
        for oxts_file in tqdm(oxts_files, total=len(oxts_files) - 1, desc='{}'.format(split)):
            oxts_data = load_oxts_data(oxts_file)
            poses = oxts_to_pose(oxts_data)

            # output
            outout_dir = os.path.join(args.output, split, 'poses')
            if not os.path.exists(outout_dir):
                os.makedirs(outout_dir)
            outout_path = os.path.join(outout_dir, oxts_file[-8:])

            np.savetxt(outout_path, poses)
