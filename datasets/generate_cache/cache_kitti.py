...
class KITTI(Dataset):
    def __init__(self, cfg, split):
        ...
        if split == 'train':
            ...
            self.sequences = cfg['dataset'].training.seq
        elif split == 'predict':
        ...
        self.sequence_path = os.path.join(self.kitti_path, 'data_odometry_color/dataset/sequences')

        # for each sequence
        for seq in tqdm(self.sequences, desc='Loading sequence: '):
            sequence_path = os.path.join(self.sequence_path, seq)
            ...
            for i in range(len(frames) - 1):
                ...
                frame_pair = {}
                frame_pair['frame1'] = FramePath(
                    frame_id=frames[i], sequence_path=sequence_path,
                    left_img_path=left_img_path1, right_img_path=right_img_path1,
                )
                frame_pair['frame2'] = FramePath(
                    frame_id=frames[i+1],  sequence_path=sequence_path,
...
def main():
    ...
    if args.seg_method == 'yolo':
        yolo = YOLO("/home/xiechen/科研/E2E_stereo_VO/v3/yolo11s-seg.pt")
        dilator = Dilate(kernel_size=5)

    else:
        raise NotImplementedError

    dataset = KITTI(cfg, split='predict')
